...
void Example02()
{
    ...
    Count(totalCount);
    //LO QUE HACE ESTE EJEMPLO ES CREA UN THREAD, LE HAGO JOIN Y SE PARA. LUEGO, CREA DOS MAS Y LOS HACE JOIN TAMBIEN
    //PARA CALCULAR ESTOS DOS ULTIMOS, NOS TENEMOS QUE ESPERAR A QUE ACABE EL PRIMERO
}
...
void Example03()
{
    ...
    Count(totalCount);
    //CREO 3 THREADS Y LOS DECLARO EN DETACH, AL LLAMAR EL COUNT SE EJECUTA MAS RAPIDO DE LO QUE TARDA EN CALCULAR LOS 3 THREADS
   //ES POR ESO QU HAY LA POSIBILIDAD DE QUE DIGA QUE EL CODIGO HA ACABADO QUE LOS THREADS NO HAN TERMINADO DE CALCULAR Y POR ESO SALEN DESPUES DEL MENSAJE DE QUE EL EJEMPLO HA ACABADO
}
...
void Example04()
{
    ...
    Count(totalCount);
    //CREO UNA LISTA DE THREADS, CREO UN BUCLE QUE ME CREA UN THREAD, LO AÑADO A LA LISTA Y LO HAGO JOIN 
    //NO ITERARA HASTA QUE NO ACABE DE CALCULAR EL THREAD CREADO
}
...
void Example05()
{
    ...
    Count(totalCount);
    //SIMILAR AL EJEMPLO 04, SALVO QUE CADA ITERACION SE HACE NORMAL EN VEZ DE ESPERAR A QUE EL THREAD ANTERIOR HAYA HECHO YA SU CALCULO
}
...
void Example06()
{
    unsigned unsigned long long maxThreads = std::thread::hardware_concurrency();//numero de threads posibles
    std::cout << maxThreads << " concurrent threads are supported." << std::endl;

    unsigned unsigned long long extraThreads = 0;//canviar el valor por uno negativo hara que haya menos threads

    std::list<std::thread*>* threads = new std::list<std::thread*>();

    unsigned long long totalCount = 1000 * 1000 + 1000000000;
    ...
    Count(totalCount);
    //dos bucles, uno donde crea los threads y otro donde los asinga un join, asi se calculan uno tras de otro
